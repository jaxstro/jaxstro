...
def assert_fresh(
    ...
    *,
    ...
    generating environment) rather than content.
    """
    ...
    _compare(recorded, fresh, "$", exact=origin == Origin.of_runner())


def _compare(recorded: Any, fresh: Any, path: str, *, exact: bool) -> None:
    ...
    if isinstance(recorded, dict):
        ...
        for key in recorded:
            _compare(recorded[key], fresh[key], f"{path}.{key}", exact=exact)
        ...
        for index, (left, right) in enumerate(zip(recorded, fresh, strict=True)):
            _compare(left, right, f"{path}[{index}]", exact=exact)
