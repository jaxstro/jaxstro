...
def test_freshness_compares_bytes_on_origin_and_size_elsewhere(monkeypatch) -> None:
    ...
    rerendered = render_webp_bytes(spec.builder(), spec=spec.export)
    same_size_other_bytes = committed[:-1] + bytes([committed[-1] ^ 1])

    monkeypatch.setattr(cli, "_on_render_origin", lambda: True)
    assert cli._is_fresh(committed, committed)
