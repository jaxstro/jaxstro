"""
Tests for the Gaussian rules and Hermite-e helpers in jaxstro.quad.
...
class TestGaussLaguerre:
    ...
    def test_returns_pair_correct_shape(self):
        nodes, weights = quad.gauss_laguerre_nodes(16)
    ...
    def test_exponential_weight_moments_exact(self):
        n = 8
        nodes, weights = quad.gauss_laguerre_nodes(n)
        for k in range(2 * n):
            approx = jnp.sum(weights * nodes**k)
...
class TestClenshawCurtis:
    ...
    def test_returns_pair_correct_shape_and_endpoints(self):
        nodes, weights = quad.clenshaw_curtis_nodes(9)
...
class TestHermiteEBasis:
    ...
    def test_orthogonality_under_gh(self):
        """<He_i He_j>_{N(0,1)} = i! delta_ij via GH quad."""
        g, w = quad.gauss_hermite_nodes(48)
        he = quad.hermite_e_basis(g, 5)
        for i in range(6):
            for j in range(6):
...
class TestHermiteCoefficients:
    """Hermite-e expansion coefficients via GH quad."""
    ...
    def test_known_expansion(self):
        ...
        c = quad.hermite_coefficients(map_fn, n_max=4, n_quad=64)
    ...
    def test_mean_term(self):
        ...
        c = quad.hermite_coefficients(lambda g: g**2, n_max=2, n_quad=32)
