...
def _m():
    return Model(a=jnp.array([1.0, 2.0]), b=jnp.array(3.0))
...
def test_from_where_equals_from_filter():
    ...
    spec = eqx.tree_at(
        lambda x: (x.a, x.b),
        jax.tree_util.tree_map(lambda _: False, eqx.filter(m, eqx.is_array)),
        replace=(True, True),
    )
...
def test_grad_through_from_vector():
    ...
    def loss(v):
        ...
        return jnp.sum(mm.a**2) + mm.b**2

    g = jax.grad(loss)(v0)
    h = 1e-6
    fd = jnp.array(
        [
            (loss(v0.at[i].add(h)) - loss(v0.at[i].add(-h))) / (2 * h)
...
def _nested():
    return Nested(
        inner=Model(a=jnp.array([1.0, 2.0]), b=jnp.array(3.0)), c=jnp.array([4.0, 5.0])
    )
...
def test_nested_module_roundtrip_and_partial_free():
    ...
    p = Parameterization.from_where(
        m, where=lambda x: (x.inner.a, x.c)
    )  # inner.b stays fixed
