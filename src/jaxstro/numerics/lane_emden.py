r"""Differentiable Lane-Emden solver: self-gravitating isothermal and polytropic spheres.

Two branches of the same self-gravitating-sphere problem, kept as two right-hand sides
rather than one parametrized family:
...
Numerics are borrowed, not hand-rolled: diffrax ``Tsit5`` with
``PIDController(rtol=1e-8, atol=1e-10)``, the same adaptive-ODE idiom used elsewhere in
the ecosystem (e.g. progenax's king/michie/limepy profiles). Gradients flow through the
solve and through the ``polytrope_xi1`` event root (implicit function theorem).
...
docs/provenance/registry/lane_emden.yaml.
"""
...
# diffrax settings -- adaptive Tsit5, matching the ecosystem's stiff-safe ODE idiom.
_RTOL = 1e-8
_ATOL = 1e-10
_DT0 = 1e-4
_MAX_STEPS = 100_000
...
def polytrope_xi1(n, xi_search_max: float = 50.0) -> Float[Array, ""]:
    ...
    theorem, whereas ``argmin`` has no gradient at all.
    ...
    ``xi_search_max``; callers must reject that regime.
