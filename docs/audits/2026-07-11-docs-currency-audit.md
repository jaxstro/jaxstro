...
| `docs/20-architecture/spectra-data-architecture.md` | Interface notation | `AtmosphereQuery -> PreparationResult[PreparedAtmosphere]` boundary | **Updated in spectra v1:** rendered as captioned interface notation, not Python |
| `docs/20-architecture/spectra-data-architecture.md` | Executable | canonical `Spectrum` plus prepared stencil, status, JIT, and gradient | **Updated in spectra v1:** the artifact-free block executes verbatim against `jaxstro.spectra` and checks finite interpolation, fail-closed outside values, and the local derivative |
| `docs/20-architecture/spectra-data-architecture.md` | Local-data example | exact `AtmosphereQuery` and `AtmosphereLibrary.prepare` | **Updated in spectra v1:** explicitly labelled as requiring local processed artifacts plus the optional data dependencies |
...
| Atmosphere runtime | Four family adapters, 27 exact TLUSTY composition products, real holdouts, and prepared-stencil transform tests | Implemented with an explicit evidence boundary: NewEra/BOSZ/OSTAR representative paths accepted; Sonora/BSTAR remain `POLICY_NOT_VALIDATED` |
...
| C11f | `docs/20-architecture/spectra-data-architecture.md` | **Superseded by spectra v1 on 2026-07-12:** canonical generic types and exact queries replace the deleted `PreparedSpectralGrid`; source semantics, topology statuses, and Fluxax ownership are taught directly | Executable page test plus real-artifact, policy-manifest, AD, interoperability, performance, and docs gates |
...
| Spectra v1 follow-up | atmosphere capability/query/how-to pages | **Completed 2026-07-12:** product matrix, query recipe, TLUSTY conversion detail, API map, and validation evidence now match runtime code | `tests/integration/test_spectra_data_architecture_docs.py`, `tests/validation/test_atmospheres_spectra.py`, strict docs gate |
