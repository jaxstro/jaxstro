...
| C0-006 | P2 | Quantity is simultaneously documented as implemented and planned/missing | `science-general-vision.md` retains pre-implementation language while theory/API/validation describe the live package | **Resolved for the landing in C3 and architecture index in C4:** both identify `jaxstro.units` as the current contract and quantity adoption/cutover as deferred; reconcile the vision in its approved page slice |
...
| C0-008 | P2 | Slice-B provenance cards are not integrated into the teaching/reference narrative | The landing and architecture index now route to the generated cards and distinguish them from runtime manifests, but the API description of `jaxstro.testing`, validation anchors, and dedicated provenance page do not yet describe card validation/rendering or evidence states | **Resolved for the landing in C3 and architecture index in C4;** reconcile the remaining API, provenance-architecture, and validation pages after their individual approvals |
