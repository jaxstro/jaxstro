...
| C0-008 | P2 | Slice-B provenance cards are not integrated into the teaching/reference narrative | The landing, architecture index, API, and validation page now route to generated cards and distinguish or validate their evidence, but the dedicated provenance page does not yet explain the ownership split | **Resolved for C3, C4, C6, and C9;** reconcile the remaining provenance-architecture page in C10 |
...
| Provenance-card tooling | `jaxstro.testing.provenance_cards`, registry validation tests, generated pages | Implemented; navigation, API, and validation integration are current; the dedicated architecture ownership narrative remains C10 |
