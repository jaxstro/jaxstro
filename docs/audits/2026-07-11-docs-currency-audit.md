...
| C0-006 | P2 | Quantity is simultaneously documented as implemented and planned/missing | `docs/20-architecture/index.md` and `science-general-vision.md` retain pre-implementation language while theory/API/validation describe the live package | **Resolved for the landing in C3:** it now identifies `jaxstro.units` as the current contract and quantity adoption/cutover as deferred; reconcile architecture and vision in their approved page slices |
...
| C0-008 | P2 | Slice-B provenance cards are not integrated into the teaching/reference narrative | The landing now routes directly to the generated card index and constants card, but the API description of `jaxstro.testing`, validation anchors, and architecture prose do not yet describe card validation/rendering or evidence states | **Resolved for the landing in C3;** reconcile the remaining API, provenance-architecture, and validation pages after their individual approvals |
...
| C4 | `docs/20-architecture/index.md` | Architecture describes the current foundation rather than a planned quantity layer | Installed-module assertions and xrefs |
| C5 | `docs/20-architecture/science-general-vision.md` | Vision cleanly separates delivered modules from future candidates | Source/module inventory check |
| C6 | `docs/40-api/index.md` | Every advertised import has explicit ownership and public evidence/provenance surfaces are discoverable | Executable import/symbol test; decide eager top-level `spatial` ownership |
| C7 | new `docs/10-theory/spatial.md` | Users understand discrete preprocessing, Morton/grid capacity, approximate recall, and exact-pair boundaries | Small deterministic spatial example plus existing recall/pair tests |
