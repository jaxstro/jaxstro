...
| C0-013 | P3 | Python fences do not expose their execution contract | Continuations, placeholders, local-data examples, and interface pseudocode all use the same unlabeled `python` fence as runnable snippets | **Resolved for B-splines in C11a:** continue one page at a time for interpolation, random, regular-grid, linear-algebra, and spectra-data architecture |
...
| C11b+ | `interpolation.md`, `random.md`, `regular-grid.md`, `linear-algebra.md`, `spectra-data-architecture.md` | Each remaining illustrative block is either completed or explicitly marked | One page and one focused proof at a time |
