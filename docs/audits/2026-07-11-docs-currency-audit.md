...
| C0-013 | P3 | Python fences do not expose their execution contract | Continuations, placeholders, local-data examples, and interface pseudocode all use the same unlabeled `python` fence as runnable snippets | **Resolved for B-splines in C11a, interpolation in C11b, random streams in C11c, regular-grid interpolation in C11d, and linear algebra in C11e:** the remaining spectra-data architecture fences require explicit interface/local-data classification |
| C0-014 | P3 | The rendered bibliography page contains a duplicate `references` ID | MyST renders both the authored “References” heading and the generated bibliography section as `id="references"`; the C8 DOM audit found two occurrences on `/index-11` | Resolve in the bibliography page slice by removing the authored/generated anchor collision, then rerun the site-wide DOM uniqueness gate |
...
| C11f | `docs/20-architecture/spectra-data-architecture.md` | Classify interface pseudocode and local-data examples honestly without pretending external artifacts are portable executable fixtures | One bounded architecture page and its focused proof |
