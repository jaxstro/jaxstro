...
| `jaxstro.numerics.map_safeguarded_bracketed_root` | validated | value_first | `jit`: supported; `vmap`: conditional (Values and shapes are preserved, but physical per-lane skipping is not guaranteed.) | Missing sign bracket or nonfinite admissible evaluation. [structured_result] | [`root.map_safeguarded_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/unit/test_bracketed_root.py) (unit_test) | No implicit-root derivative claim.; Use lax.map when physical per-lane skipping of expensive residuals matters. |
...
| `jaxstro.numerics.safeguarded_bracketed_root` | validated | value_first | `jit`: supported; `vmap`: conditional (Values and shapes are preserved, but physical per-lane skipping is not guaranteed.) | Missing sign bracket or nonfinite admissible evaluation. [structured_result] | [`root.safeguarded_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/unit/test_numerics.py) (unit_test) | No implicit-root derivative claim.; Use lax.map when physical per-lane skipping of expensive residuals matters. |
