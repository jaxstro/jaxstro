...
| --- | --- | --- | --- | --- | --- | --- |
| `jaxstro.numerics.implicit_bracketed_root` | validated | certified_implicit | `jit`: supported | Rejected assumption or numerical certificate. [returns_nan] | [`root.implicit_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/validation/test_implicit_root_gradients.py) (validation_test) -> [`rootfinding.implicit-gradients`](../evidence-index) | Requires a unique mathematical root.; Requires a smooth selected branch and adequate slope conditioning.; Runs the safeguarded primal before exposing a custom-root derivative. |
...
| `jaxstro.numerics.map_safeguarded_bracketed_root` | validated | value_first | `jit`: supported; `vmap`: conditional (Values and shapes are preserved, but physical per-lane skipping is not guaranteed.) | Missing sign bracket or nonfinite admissible evaluation. [structured_result] | [`root.map_safeguarded_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/unit/test_bracketed_root.py) (unit_test) -> [`rootfinding.performance`](../evidence-index) | No implicit-root derivative claim.; Use lax.map when physical per-lane skipping of expensive residuals matters. |
...
| `jaxstro.numerics.safeguarded_bracketed_root` | validated | value_first | `jit`: supported; `vmap`: conditional (Values and shapes are preserved, but physical per-lane skipping is not guaranteed.) | Missing sign bracket or nonfinite admissible evaluation. [structured_result] | [`root.safeguarded_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/unit/test_numerics.py) (unit_test) -> [`rootfinding.performance`](../evidence-index) | No implicit-root derivative claim.; Use lax.map when physical per-lane skipping of expensive residuals matters. |
