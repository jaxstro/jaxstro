...
| Random stream and resampling helpers preserve deterministic key and shape contracts | Exact fixed-key behavior, degenerate distribution checks, exact residual integer counts, and JIT shape checks | Key streams, folded-in streams, seed manifests, systematic/stratified/residual resampling | `tests/unit/test_random.py` |
...
| Distribution kernels preserve normalization, monotone CDFs, inverse-CDF round trips, and smooth interior gradients | Numerical integration tolerances, exact round trips within floating tolerance, and FD-vs-AD checks away from support edges | Normal, lognormal, finite power law, and truncated normal logpdf/CDF/PPF helpers | `tests/unit/test_distributions.py`, `tests/validation/test_grad_checks.py` |
...
| Atmospheres remain in progress; current data indexing does not vendor raw PHOENIX data | Fixture size guard and parser-only tests | Synthetic tiny NewEra-like files only; this row does not claim complete backend coverage | `tests/unit/test_atmospheres.py` |
