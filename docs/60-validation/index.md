...
| Root bracketing and monotone inverse tables expose explicit value/gradient contracts | Exact synthetic roots, fixed trace masks, explicit exhaustion/missing-bracket flags, and FD-vs-AD checks only where claimed | Fixed-count bracket expansion, independent bisection arrays, safeguarded secant/midpoint proposals, no post-convergence evaluations, JIT/VMAP scalar solves, clamped monotone inverse interpolation, and documented value-first AD policy | `tests/unit/test_numerics.py`, `tests/validation/test_grad_checks.py` |
