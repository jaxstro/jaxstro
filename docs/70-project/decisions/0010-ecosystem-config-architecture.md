...
- **No dependency cycle (the hard blocker):** cross-package experiments import gravax/progenax/fluxax; core is imported *by* those. A hydra lab inside the core distribution -> `core -> siblings -> core` cycle. The lab must sit *above* core. (A repository boundary is not the same as a distribution boundary: a lab MAY live in the jaxstro repo, but MUST be a separate distribution above core. Anna chose a dedicated **`jaxstro-lab` repo**.)
