...
**Use Diátaxis (Tutorial, Explanation, How-To, Reference) as the spine for user-facing quadrants.** Add first-class meta sections adjacent to architecture: numbered ADR log (70-project/decisions), validation tables (60-validation), and development log (70-project/development). Include an explicit dual-front-door landing so users can enter via "Learn the methods" or "Look up the API."
...
- **Knowledge-web**: narrative (10-theory) bridges to atomic decisions (70-project/decisions) bridges to API (40-api) - creates a high-fidelity onboarding path for research students unfamiliar with the codebase.
