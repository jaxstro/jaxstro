...
| `jaxstro.numerics.safeguarded_bracketed_root` | validated | value_first | `jit`: supported; `vmap`: conditional (Values and shapes are preserved, but physical per-lane skipping is not guaranteed.) | Missing sign bracket or nonfinite admissible evaluation. [structured_result] | [`root.safeguarded_bracketed_root`](https://github.com/drannarosen/jaxstro/blob/main/tests/unit/test_numerics.py) (unit_test); [`root.safeguarded_bracketed_root.public-validation`](https://github.com/drannarosen/jaxstro/blob/main/tests/validation/test_bracketed_root_algorithms.py) (validation_test); [`root.safeguarded_bracketed_root.performance`](https://github.com/drannarosen/jaxstro/blob/main/scripts/benchmark_rootfinding.py) (benchmark) -> [`rootfinding.performance`](../../60-validation/evidence-index.md) gates `flat_slope.hybrid-no-more-evaluations, linear.hybrid-no-more-evaluations, monotone_kink.hybrid-no-more-evaluations, oscillatory_fixed_point_residual.hybrid-no-more-evaluations, quadratic.hybrid-no-more-evaluations` | No implicit-root derivative claim.; Use lax.map when physical per-lane skipping of expensive residuals matters. |
