...
next: Write the fresh `jaxstro.quad` Phase A2 adaptive-rule implementation plan against the verified A1 interfaces, then execute it inline with read-only subagent reviews at checkpoints. A2 remains primal-only and raw-array-only; do not begin replay derivatives, quantity integration, sibling migrations, deprecations, publication, or push.
