...
def _source_members(raw_zip: Path, limit: int | None = None) -> list[str]:
    with zipfile.ZipFile(raw_zip) as archive:
        members = [
            ...
            if name.endswith(".spec") and parse_sonora_2024_filename(name) is not None
        ]
    members = sorted(
        members,
        key=lambda name: (
            parse_sonora_2024_filename(name).m_h,
            parse_sonora_2024_filename(name).c_o,
            parse_sonora_2024_filename(name).teff,
            parse_sonora_2024_filename(name).logg,
            parse_sonora_2024_filename(name).cloud_label,
            name,
        ),
    )
...
def _inspect_members(raw_zip: Path, members: list[str]) -> SonoraInspection:
    ...
    with zipfile.ZipFile(raw_zip) as archive:
        ...
        for member in members[1:]:
            other, _, _ = _read_spectrum_member(archive, member)
            if other.shape != wavelength.shape or not np.allclose(other, wavelength):
                raise ValueError(
...
def convert_sonora_zip(
    ...
    *,
    ...
    members = _source_members(raw_zip, limit=limit)
    ...
    with zipfile.ZipFile(raw_zip) as archive:
        for row, member in enumerate(members):
            ...
            _, flux64, header = _read_spectrum_member(archive, member)
...
def main() -> None:
    ...
    members = _source_members(args.raw_zip, limit=args.limit)
    if args.dry_run:
        print(f"Would convert {len(members)} Sonora spectra")
