"""Validate and emit the executable curriculum registry."""

from __future__ import annotations

import argparse
import json
from pathlib import Path
from typing import Any

ROOT = Path(__file__).resolve().parents[1]
MANIFEST = "docs/curriculum/units.json"
UNIT_FIELDS = {
    "id",
    "title",
    "page",
    "example",
    "prerequisites",
    "public_apis",
    "contract_ids",
    "evidence_ids",
    "validation_targets",
    "limitations",
    "instructor_route",
}


def validate_instructor_route(root: Path, route: str) -> None:
    """Require an instructor route to resolve to a committed page."""
    if not route or not (root / route).is_file():
        raise ValueError(f"instructor route does not exist: {route}")


def load_and_validate_units(root: Path) -> list[dict[str, Any]]:
    """Load units and fail closed on references or schema drift."""
    payload = json.loads((root / MANIFEST).read_text(encoding="utf-8"))
    if set(payload) != {"schema_version", "units"} or payload["schema_version"] != "1":
        raise ValueError("unsupported curriculum manifest schema")
    contracts = json.loads(
        (root / "docs/validation/contracts.json").read_text(encoding="utf-8")
    )
    contract_records = {
        item["id"]: item
        for module in contracts["modules"]
        for item in module["callables"]
    }
    evidence = json.loads(
        (root / "docs/validation/evidence-index.json").read_text(encoding="utf-8")
    )
    evidence_ids = {item["id"] for item in evidence["entries"]}
    seen: set[str] = set()
    units = sorted(payload["units"], key=lambda item: item["id"])
    for unit in units:
        if set(unit) != UNIT_FIELDS:
            raise ValueError(f"curriculum unit fields mismatch: {unit.get('id', '?')}")
        identity = unit["id"]
        if identity in seen:
            raise ValueError(f"duplicate curriculum unit id: {identity}")
        seen.add(identity)
        for field in ("id", "title", "page", "example"):
            if not isinstance(unit[field], str) or not unit[field].strip():
                raise ValueError(f"curriculum {field} must be nonempty: {identity}")
        for field in (
            "prerequisites",
            "public_apis",
            "contract_ids",
            "evidence_ids",
            "validation_targets",
            "limitations",
        ):
            if not isinstance(unit[field], list) or not all(
                isinstance(value, str) and value.strip() for value in unit[field]
            ):
                raise ValueError(f"curriculum {field} must be text list: {identity}")
        if not unit["contract_ids"] or not unit["validation_targets"]:
            raise ValueError(
                f"curriculum unit lacks contract or validation: {identity}"
            )
        missing_contracts = set(unit["contract_ids"]) - set(contract_records)
        if missing_contracts:
            raise ValueError(
                f"unknown curriculum contract ids: {sorted(missing_contracts)}"
            )
        contract_paths = {
            contract_records[contract_id]["import_path"]
            for contract_id in unit["contract_ids"]
        }
        if set(unit["public_apis"]) != contract_paths:
            raise ValueError(f"curriculum API and contract paths disagree: {identity}")
        missing_evidence = set(unit["evidence_ids"]) - evidence_ids
        if missing_evidence:
            raise ValueError(
                f"unknown curriculum evidence ids: {sorted(missing_evidence)}"
            )
        for field in ("page", "example"):
            if not (root / unit[field]).is_file():
                raise ValueError(f"curriculum {field} does not exist: {unit[field]}")
        for field in ("prerequisites", "validation_targets"):
            for target in unit[field]:
                if not (root / target).is_file():
                    raise ValueError(f"curriculum target does not exist: {target}")
        validate_instructor_route(root, unit["instructor_route"])
    return units


def _render_index(units: list[dict[str, Any]]) -> str:
    lines = [
        "---",
        "title: Executable research investigations",
        "description: Predict, compute, audit, and state the warranted claim using public Jaxstro APIs.",
        "---",
        "",
        "# Executable research investigations",
        "",
        "Each investigation imports public APIs from one repository-owned Python module. The page teaches the reasoning; the example is the executable source of truth.",
        "",
        "| Investigation | Public contracts | Indexed evidence | Known limitations |",
        "| --- | --- | --- | --- |",
    ]
    for unit in units:
        page = Path(unit["page"]).name
        contracts = ", ".join(f"`{item}`" for item in unit["contract_ids"])
        evidence = (
            ", ".join(f"`{item}`" for item in unit["evidence_ids"])
            or "No standalone indexed artifact; callable validation only"
        )
        limitations = "; ".join(unit["limitations"])
        lines.append(
            f"| [{unit['title']}](./{page}) | {contracts} | {evidence} | {limitations} |"
        )
    lines.extend(
        [
            "",
            "Every command prints measured results only as:",
            "",
            "| Metric identity | Symbol | Value | Units |",
            "| --- | --- | ---: | --- |",
            "| example metric | `m` | measured | explicit units |",
            "",
            "The curriculum registry is checked against the scientific contract registry and evidence index. Missing indexed evidence remains visible rather than being inferred from a passing example.",
            "",
        ]
    )
    return "\n".join(lines)


def _render_coverage(units: list[dict[str, Any]]) -> str:
    payload = {
        "schema_version": "1",
        "unit_count": len(units),
        "units": [
            {
                "contract_count": len(unit["contract_ids"]),
                "evidence_count": len(unit["evidence_ids"]),
                "id": unit["id"],
                "instructor_route": unit["instructor_route"],
                "limitation_count": len(unit["limitations"]),
                "validation_target_count": len(unit["validation_targets"]),
            }
            for unit in units
        ],
    }
    return json.dumps(payload, indent=2, sort_keys=True) + "\n"


def render_outputs(root: Path = ROOT) -> dict[Path, str]:
    """Render every generated curriculum product from one validated manifest."""
    units = load_and_validate_units(root)
    return {
        root / "docs/70-investigations/investigations.md": _render_index(units),
        root / "docs/validation/curriculum-coverage.json": _render_coverage(units),
    }


def main() -> int:
    parser = argparse.ArgumentParser()
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("--emit", action="store_true")
    mode.add_argument("--check", action="store_true")
    args = parser.parse_args()
    outputs = render_outputs()
    if args.emit:
        for path, content in outputs.items():
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(content, encoding="utf-8")
        print("curriculum registry emitted")
        return 0
    stale = [
        path
        for path, content in outputs.items()
        if not path.is_file() or path.read_text(encoding="utf-8") != content
    ]
    if stale:
        for path in stale:
            print(f"stale: {path.relative_to(ROOT)}")
        return 1
    print("curriculum registry fresh")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
